...
private:
    ...
    unordered_map<RubiksCube, CubeBase::MOVE, HashCube> move_done;
    ...
    struct Node {
        ...
        Node(RubiksCube _cube, int _depth, int _estimate) : cube(_cube), depth(_depth), estimate(_estimate) {};
    };
...
// returns {rubiksCube, next_bound}, if the cube was not solved
    pair<RubiksCube, int> IDAstar(int bound) {
...
public:
    ...
    IDAstarSolver(RubiksCube _rubiksCube, string fileName) {
    ...
    vector<RubiksCube::MOVE> solve() {
        ...
        RubiksCube solved_cube = p.first;
        assert(solved_cube.isSolved());
        RubiksCube curr_cube = solved_cube;
